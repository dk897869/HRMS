...
const KPIWidget = ({ title, value, subtitle, icon: Icon, color, loading, onClick, isActive }) => (
  <GlassCard onClick={onClick} sx={{ 
    p: 3.5, display: 'flex', alignItems: 'center', gap: 3, flex: 1, minWidth: 240,
    ...
    <Box sx={{ flexGrow: 1, minWidth: 0 }}>
      <Typography variant="body2" sx={{ color: '#A3AED0', fontWeight: 700, mb: 0.5, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: 0.5 }}>{title}</Typography>
      {loading ? <Skeleton width={80} height={40} /> : (
        <Typography variant="h4" sx={{ color: '#1B254B', fontWeight: 900, lineHeight: 1 }}>{value}</Typography>
      )}
      {subtitle && <Typography variant="caption" sx={{ color: '#A3AED0', fontWeight: 600, mt: 0.8, display: 'block' }}>{subtitle}</Typography>}
    </Box>
  </GlassCard>
);
...
const AdminDashboard = () => {
  ...
  const rowsPerPage = 12; // Increased since we have more vertical space now
  ...
  const handleCorrectionSubmit = async (e) => {
    ...
    try {
      ...
      fetchAttendance(); // Re-fetch to get dynamic updates from DB
    } catch (err) {
  ...
  const handleUpdateStatus = async (newStatus) => {
    ...
    try {
      ...
      fetchAttendance(); // Re-fetch to apply dynamically
    } catch (err) {
  ...
  const filteredRecords = attendanceRecords.filter(r => {
    ...
    const st = statusFilter === 'All Status' || r.status === statusFilter;
    return s && d && st;
  });
  ...
  return (
    <Box sx={{ width: '100%', minHeight: '100vh', bgcolor: '#F4F7FE', p: { xs: 2, md: 4 } }}>
      ...
      {/* --- Premium Header --- */}
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, gap: 3, mb: 5 }}>
        <Box>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 5 }}>
        <KPIWidget 
          title="Total Employees" value={totalCount} subtitle="All registered accounts" color="#4318FF" icon={PeopleOutlinedIcon} loading={statsLoading} 
          onClick={() => handleKpiClick('All Status')} isActive={statusFilter === 'All Status'}
        />
        <KPIWidget 
          title="Present Today" value={presentCount} subtitle="Currently checked in" color="#05CD99" icon={HowToRegIcon} loading={tableLoading} 
          onClick={() => handleKpiClick('PRESENT')} isActive={statusFilter === 'PRESENT'}
        />
        <KPIWidget 
          title="On Leave" value={onLeaveCount} subtitle="Approved time off" color="#FFCE20" icon={BeachAccessIcon} loading={tableLoading} 
          onClick={() => handleKpiClick('ON_LEAVE')} isActive={statusFilter === 'ON_LEAVE'}
        />
        <KPIWidget 
          title="Absent" value={absentCount} subtitle="Not checked in today" color="#EE5D50" icon={CheckCircleOutlinedIcon} loading={tableLoading} 
          onClick={() => handleKpiClick('ABSENT')} isActive={statusFilter === 'ABSENT'}
        />
        <KPIWidget 
          title="On Break" value={onBreakCount} subtitle="Active break sessions" color="#39B8FF" icon={AccessTimeIcon} loading={statsLoading} 
          onClick={() => handleKpiClick('ON_BREAK')} isActive={statusFilter === 'ON_BREAK'}
        />
      </Box>

      {/* --- Master Table (Full Width) --- */}
      <GlassCard id="attendance-table" sx={{ p: 4, minHeight: '600px' }}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', mb: 4, gap: 3 }}>
          <Box>
            <Typography variant="h5" sx={{ color: '#1B254B', fontWeight: 900, mb: 0.5 }}>Today's Attendance</Typography>
            <Typography variant="body2" sx={{ color: '#A3AED0', fontWeight: 600 }}>Real-time check-in directory for all employees</Typography>
          </Box>
          
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <TextField 
              placeholder="Search by name, ID or email..." size="medium"
              value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
              InputProps={{
                startAdornment: <InputAdornment position="start"><SearchIcon sx={{ color: '#A3AED0' }} /></InputAdornment>,
                sx: { borderRadius: '16px', bgcolor: '#F4F7FE', '& fieldset': { border: 'none' }, minWidth: 280, fontWeight: 600, color: '#1B254B' }
              }}
            />
            <Select size="medium" value={deptFilter} onChange={(e) => { setDeptFilter(e.target.value); setPage(1); }}
              sx={{ borderRadius: '16px', bgcolor: '#F4F7FE', color: '#1B254B', fontWeight: 700, '& fieldset': { border: 'none' }, minWidth: 180 }}>
              {uniqueDepts.map(d => <MenuItem key={d} value={d} sx={{ fontWeight: 600 }}>{d}</MenuItem>)}
            </Select>
            <Select size="medium" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
              sx={{ borderRadius: '16px', bgcolor: '#F4F7FE', color: '#1B254B', fontWeight: 700, '& fieldset': { border: 'none' }, minWidth: 160 }}>
        ...
        <TableContainer sx={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid #F4F7FE' }}>
          <Table>
            <TableHead sx={{ bgcolor: '#F8FAFC' }}>
              <TableRow>
                {['EMPLOYEE', 'EMAIL', 'DEPARTMENT', 'CHECK IN', 'CHECK OUT', 'HOURS', 'STATUS', 'LOCATION', 'ACTIONS'].map(th => (
              ...
              {tableLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} sx={{ textAlign: 'center', py: 10 }}>
                     <Typography variant="h6" sx={{ color: '#A3AED0', fontWeight: 700 }}>No attendance records found matching your filters.</Typography>
                     <Button variant="text" onClick={() => { setSearch(''); setDeptFilter('All Departments'); setStatusFilter('All Status'); }} sx={{ mt: 2, color: '#4318FF', fontWeight: 800, textTransform: 'none' }}>Clear Filters</Button>
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={r.id} sx={{ '&:hover': { bgcolor: '#F9FAFD' }, transition: 'background 0.2s' }}>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE', py: 2.5 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
                        ...
                        <Box>
                          <Typography variant="subtitle1" sx={{ color: '#1B254B', fontWeight: 800 }}>{r.name}</Typography>
                          <Typography variant="caption" sx={{ color: '#A3AED0', fontWeight: 700 }}>{r.empId}</Typography>
                        </Box>
                      </Box>
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE', color: '#A3AED0', fontWeight: 600, fontSize: '0.85rem' }}>
                      {r.email}
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE' }}>
                       <Chip label={r.dept} sx={{ bgcolor: '#F4F7FE', color: '#4318FF', fontWeight: 800, borderRadius: '10px', height: 28, fontSize: '0.75rem' }} />
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE', color: r.checkIn !== '—' ? '#01B574' : '#A3AED0', fontWeight: 800, fontSize: '0.9rem' }}>
                      {r.checkIn}
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE', color: r.checkOut !== '—' ? '#4318FF' : '#A3AED0', fontWeight: 800, fontSize: '0.9rem' }}>
                      {r.checkOut}
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE', color: '#1B254B', fontWeight: 900, fontSize: '0.9rem' }}>
                    ...
                    <TableCell sx={{ borderBottom: '1px solid #F4F7FE' }}>
                      <IconButton onClick={(e) => handleMenuOpen(e, r)} sx={{ color: '#1B254B', bgcolor: '#F4F7FE', borderRadius: '12px', p: 1, '&:hover': { bgcolor: '#E9EDF7' } }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 4 }}>
          ...
          <Pagination count={Math.ceil(filteredRecords.length / rowsPerPage)} page={page} onChange={(_, v) => setPage(v)} 
            sx={{ '& .MuiPaginationItem-root': { color: '#1B254B', fontWeight: 800, fontSize: '0.9rem' }, '& .Mui-selected': { bgcolor: '#4318FF !important', color: '#fff' } }} />
        </Box>
      </GlassCard>
      ...
      {/* --- View Modal (Petpooja Graphical Style) --- */}
